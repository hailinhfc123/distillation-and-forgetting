...
import json
from ewc_utils import preprocess_function_summary, preprocess_function_race
...
def batch_generator(model, data_loader, max_new_tokens, batch_size):
...
def generate_summary_outputs(model_name, dataset, path, batch_size, num_rows=1000):
    ...
    return_dict = {"label_ids": batch_generator(model, dataset_loader,max_new_tokens=max_target_length, batch_size=batch_size)}
...
def generate_race_outputs(model_name, dataset, name, path, batch_size, num_rows=1000):
    model = AutoModelForSeq2SeqLM.from_pretrained(model_name, cache_dir=cache_dir).to(device)
    race_datapoints = load_dataset(dataset, name, cache_dir=cache_dir)
    ...
    if num_rows == "full":
        ...
        train_race_set = tokenized_race["train"].select(range(num_rows))

    dataset_loader = DataLoader(train_race_set, batch_size=batch_size)
    return_dict = {"label_ids": batch_generator(model, dataset_loader,max_new_tokens=1, batch_size=batch_size)}
    dataset = Dataset.from_dict(return_dict)
    torch.save(dataset, path)
    return dataset
...
dataset = generate_race_outputs(model_large, "race", "all", "./output_race_from_t5_large_full.pt", batch_size=100, num_rows="full")
